...
class Files(GraphResource):
    """Items in a OneDrive: the signed-in person's, or ``user``'s.

    Every method takes either a drive path (``"/reports/q3.xlsx"``, leading slash) or an item id,
    and ``user`` (an id or mail address). Under application access there is no signed-in person,
    so pass it: ``upload("q3.xlsx", user="finance@contoso.com")`` writes to that person's drive.
    """
    ...
    @staticmethod
    def _address(item: str, user: Optional[str] = None) -> str:
        """The endpoint for one item, by path or by id, in ``user``'s drive when given."""
        if not item or not item.strip():
            raise GraphError(0, "invalidRequest", "'item' is required")
        drive = GraphResource._for(DRIVE, user)
    ...
    async def upload(
        self, source: Union[str, Path], to: Optional[str] = None, user: Optional[str] = None
    ) -> Dict[str, Any]:
        ...
        return await self._client.upload(
            f"{self._address(destination, user)}/content", str(local)
        )

    async def download(
        self, item: str, dest_path: Union[str, Path], user: Optional[str] = None
    ) -> Dict[str, Any]:
        ...
        return await self._client.download(
            f"{self._address(item, user)}/content", str(dest_path)
        )
    ...
    def folder(
        ...
        return self._client.paged(
            f"{self._address(path, user)}/children", select=select, top=top
        )

    def search(
        self, query: str, select: str = DEFAULT_FIELDS, user: Optional[str] = None
    ) -> AsyncIterator[Dict[str, Any]]:
        ...
        drive = self._for(DRIVE, user)
        return self._client.paged(f"{drive}/root/search(q='{escaped}')", select=select)

    async def metadata(
        self, item: str, select: str = DEFAULT_FIELDS, user: Optional[str] = None
    ) -> Dict[str, Any]:
        """One item's properties, without its bytes."""
        return await self._client.get(self._address(item, user), select=select)

    # ── changing things ──────────────────────────────────────────────────────

    async def make_folder(
        self, path: str, conflict: str = "fail", user: Optional[str] = None
    ) -> Dict[str, Any]:
        ...
        return await self._client.post(f"{self._address(parent or '/', user)}/children", body={
            "name": name,
            "folder": {},
            "@microsoft.graph.conflictBehavior": conflict,
        })

    async def remove(self, item: str, user: Optional[str] = None) -> None:
        """Move an item to the recycle bin."""
        await self._client.delete(self._address(item, user))
    ...
    async def share_link(
        ...
        created = await self._client.post(
            f"{self._address(item, user)}/createLink", body={"type": kind, "scope": scope}
        )
